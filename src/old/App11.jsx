...
// Универсальная функция для ввода данных юзера. Воровство в другой функции 
  ...
  temporaryUserData = userData.map((rec, index) => {   
    if (responseData==undefined) {return rec} // Если нет данных от бэкенда, то возвращаем исходные данные (userData
    // if (responseData.user[rec.dataName]!==undefined) {console.log('PersonalData 00 ===============>>>', 'index', index, responseData.user[rec.dataName]);
    //   return rec.value = responseData.user[rec.dataName]}
    //   // else {rec.value = ''; console.log('PersonalData 00 ===============>>>', 'index', index, responseData.user[rec.dataName])}
    // return responseData.user[rec.dataName]!==undefined ? responseData.user[rec.dataName] : ''

    if (responseData.user[rec.dataName]!==undefined) {console.log('PersonalData 00 yes ===============>>>', 'index', index, responseData.user[rec.dataName]);
      return {...rec, value:responseData.user[rec.dataName]}}
        else {console.log('PersonalData 00 else ===============>>>', 'index', index, responseData.user[rec.dataName])
    return rec}
  ...
  console.log('PersonalData 01 ===============>>>', temporaryUserData);
  const [validated, setValidated] = useState(false);

  // console.log('PersonalData 05 ===============>>>', responseData==undefined ? 'responseData еще нет' : responseData.user);
