...
// Универсальная функция для ввода данных юзера и кражи
  ...
  const handleSubmit = (event) => {
    ...
    if (form.checkValidity()) {
      ...
      let sendingData = sendData.filter((rec) => (sendIt[rec.i][requestType]==true)).map(rec => {  
            const container = {};
      ...
      console.log('************** Здесь должен быть датасендер stealData', stealData);
      ...
      .then(
        responseJson => {
          ...
          if (responseJson.status !== 'ERR') { // Ошибок нет, возвращаем данные, если они есть
            ...
            if (['SignIn', 'Auth', 'CreateOfficer', 'UpdateOfficer', 'GetOfficer'].includes(requestType)) {
              ...
              setUserData(userData.map((rec) => {   
                ...
                if (responseJson.data[rec.dataName]==undefined) return rec // Если нет данных в .user, то возвращаем исходные данные userData
            ...
            if (['CreateCase', 'CreateCasePublic', 'EditCase', 'GetOneCase'].includes(requestType)) {
              ...
              setStealData(stealData.map((rec) => {   
                console.log('DataSender requestType 3', responseJson.data[rec.dataName]);
                  if (responseJson.data[rec.dataName]==undefined) return rec // Если нет данных то возвращаем исходные данные из userData ??????????
                      else return {...rec, value:responseJson.data[rec.dataName]}
                  }));
              }
            ...
            // if (responseJson.officers) setResponseData(responseJson.officers);
            // console.log('Данные получены responseData', responseData);
    ...
    return (
      <> 
        <Table striped bordered hover variant="secondary" expand="lg" style={{ marginTop: 129 }}>
          ...
          <tbody>
            {
              allOfficersData &&    //  Надо заемнить responseData на новую переменную
                  ...
                  return (
                    ...
                    onClick={() => { 
                                                    ...
                                                    GetData('GetOfficer', // Лучше бы переменную requestType передавать, но она не успевает обновиться
                                                            ...
                                                            rec._id)
                                                    .then(() => {
                                                      console.log('MainTable 6', 'responseData', responseData);
                                                      handleShow();  
...
function App() {
 ...
 // Данные полученные от бэкенда
  ...
  const [userData, setUserData] = useState(JSON.parse(window.localStorage.getItem('userData')) ||
    ...
    //     setUserData(userData.map((rec) => {   
    //       console.log('PersonalData officersData rec.dataName', responseData[rec.dataName]);
    //         if (responseData[rec.dataName]==undefined) return rec // Если нет данных то возвращаем исходные данные из userData
    //             else return {...rec, value:responseData[rec.dataName]}
    //         }));
    //     } 
