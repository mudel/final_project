...
// Хорошая версия до начада работы с таблицами
...
async function DataSender(sendingData, 
  ...
  // Универсальная процедура отправки данных на бэкенд и получения ответа
    ...
    // Собственно функция-запрос к бэкенду
      ...
      .then((responseJson) => {
        if (responseJson.status === 'ERR' || responseJson.status == null) { // Ошибка бэкенда, вывод сообщения в модальное окно
          ...
          handleShowAlert(); // Вызов модального окна
            ...
            return responseJson.data;} // Ошибок нет, обработка data 
      }) 
