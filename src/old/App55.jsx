...
// Универсальная функция для ввода данных юзера. Воровство в другой функции 
  ...
  const handleSubmit = (event) => {
    ...
    if (form.checkValidity()) {
      ...
      let sendingData = user.filter((rec) => rec[requestType]===true).map(rec => {  
            const container = {};
  ...
  <>
    ...
    <Modal.Body>
      <Form noValidate validated={validated} onSubmit={handleSubmit}>
        ...
        {user.filter((rec) => rec.needInput).map((rec) => (
    ...
    const response = await fetch(url, query)
      ...
      .then((responseJson) => {
        if (responseJson.status === 'ERR') { // Ошибка бэкенда, вывод сообщения в модальное окно
          console.log('DataSender *** Ошибка:', responseJson.message, responseJson.errCode);
          ...
          else {setResponseData(responseJson.data)} // Ошибок нет, тут будет обработка data
      }) 
      .catch((error) => {console.log('DataSender ***** message', error.message) // Прочие ошибки
          setMessage(error.message); // Передача сообщения fetch об ошибке в модальное окно
          handleShowAlert(); // Вызов модального окна
    }); 
    console.log('DataSender ***** setResponseData',responseData);
  }
  ...
  const [user, setUser] = useState([
    {
      ...
      needInput: true, // boolean наличие свойства в меню ввода
      required: true, // boolean обязательное поле ?
      SignIn: true, // boolean наличие свойства в отправке на сервер
      SignUp: true // boolean наличие свойства в отправке на сервер
    },
    {
      ...
      needInput: false, // boolean наличие свойства в меню ввода
      required: false, // boolean обязательное поле ?
      SignIn: false, // boolean наличие свойства в отправке на сервер
      SignUp: true // boolean наличие свойства в отправке на сервер
    },
    {
