...
const handleSubmit = (event) => {
  ...
  if (form.checkValidity()) {
    ...
    console.log('PersonalData 4 ===============>>>', sendingData);

    // Когда окно закрывается данные должны отправляться на бэкенд
    DataSender(sendingData, handleShowAlert, setMessage);
    }
  console.log('PersonalData 2 ===============>>>', validated);
  };
...
async function DataSender(props) {
  // Обмен с бэкендом
  // Добавление кражи с авторизацией

  const steal = props.steal;
  const handleShowAlert = props.handleShowAlert; 
  const setMessage = props.setMessage;
    ...
    console.log('DataSender вызываем setmessage');
  setMessage('Тестовое сообщение');
  console.log('DataSender вызывали setmessage');
    ...
    const response = await fetch(url, query)
      ...
      .then((responseJson) => {
        if (responseJson.status === 'ERR') { // Ошибка бэкенда, вывод сообщения в модальное окно
          console.log('Ошибка:', responseJson.message, responseJson.errCode);
          ...
          else {console.log('------>data:', responseJson.data)} // Ошибок нет, тут будет обработка data
      }) 
      .catch((error) => {console.log('message', error.message) // Прочие ошибки
...
// Добавление кражи с авторизацией
  ...
  const url = 'https://sf-final-project-be.herokuapp.com/api/cas/' // Ошибка в адресе
  const choiceMethod = 'POST';  
...
// Модальное окно с сщщбщением об ошибке
  const showAlert = props.showAlert;
  const handleCloseAlert = props.handleCloseAlert;
  const message = props.message;
...
// Главная страница залогиненого юзера
  ...
  const handleSubmit = (event) => {
    const form = event.currentTarget;
    console.log('1===============>>>', 'checkValidity', form.checkValidity())
    if (form.checkValidity() === false) {
      event.preventDefault();
      event.stopPropagation();
    }
    ...
    if (form.checkValidity()) {
      handleClose();
      console.log('3===============>>>', steal);   // Когда окно закрывается должны отправляться данные на бэкенд
